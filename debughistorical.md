...
### Run #10 — correction bâtie, essai humain encore nécessaire
https://github.com/greenpower2669/FabMap/actions/runs/35783702585 : extraction des 24 sprites, javac/Gradle et Release réussis. v0.5.2-b10, 811831 octets. Aucun test humain de l'alpha, des ombres sur téléphone, ni des ZIP PNG/JPEG ; sauvegarde complète avant remplacement de signature debug.

## 0.6.0 — écarts de compréhension corrigés et risques à vérifier
Fab précise : le crayon arc-en-ciel sert SEULEMENT au sens/couleur personnelle ; l'engrenage local donne les options de la bulle ; l'engrenage général à l'accueil donne sauvegarde/restauration complète. Le + est sous la bulle et rattache la fille à la bulle touchée (pas au contexte global). Le label exact de sortie devient « Exporter cette bulle et ses filles (avec leurs fichiers liés) » ; l'import local préserve l'existant.
La représentation d'une bulle doit exister même sans icon externe : photo > pictogramme > symbole neutre, y compris dans la Vallée ; ne jamais utiliser par défaut l'icône Maison pour un autre concept. BubblePacks.photoName acceptait seulement JPG et aurait rejeté une photo de représentation PNG : corrigé vers JPG/PNG sans altérer les vieilles archives. Les champs cognitifs sont des données personnelles explicitement remplies, pas des sentiments inférés. Risques : contraste de couleurs imposées, hit targets Canvas à faible zoom, menus/appuis longs, mémoire média, sauvegarde/import, partage de nœud, variante d'APK signée debug. CI n'est pas test tactile/ergonomique.

### Run #11 — succès et limites
https://github.com/greenpower2669/FabMap/actions/runs/35853316638 : extraction 24 PNG, BUILD SUCCESSFUL, Release v0.6.0-b11 (819771 octets). Aucun test visuel sur téléphone, tactile Canvas, sauvegarde et import JPG/PNG instrumenté ; garder la prudence debug et ne pas annoncer une validation de ces usages.
