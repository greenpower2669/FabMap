...
### Livraison 0.5.2 — compilation vérifiée
La v0.5.2-b10 (24 sprites PNG alpha, rendu sobre et import PNG personnel) est publiée après build Android réussi ; la qualité visuelle et le parcours d'import/restauration attendent le test de Fab sur téléphone. Ne pas rétablir le cartouche massif par défaut.

## 0.6.0 — Décision Fab : interface par bulle et sémantique cognitive (prioritaire)
- Chaque bulle, en parcours guidé ET dans la Vallée à zoom suffisant, affiche représentation (photo prioritaire, sinon pictogramme, sinon symbole neutre), titre lisible et trois actions : 🌈✏ crayon pour couleur et évocations personnelles ; ＋ EN DESSOUS pour créer sa fille ; ⚙ pour ses options. Le corps s'ouvre au tap, le B vocal demeure au long press ; au petit zoom Canvas les commandes restent accessibles en guidé.
- Le crayon propose grands boutons en HEXAGONES nommés ET code #RRGGBB libre. Champs séparés sentiments, évocations/souvenirs. L'utilisateur choisit le sens personnel et sa couleur ; aucune IA n'infère sa vie affective. Title/description/étapes/liens restent inchangés, les nouvelles clés cognitiveColor, cognitiveFeelings, cognitiveEvocation s'ajoutent au même nœud, y compris partagé.
